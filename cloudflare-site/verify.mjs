...
const assertions = [
  ...
  [!domainArtifacts.includes('sovereigntank.com'), 'no stale .com canonical/domain references'],
  [html.includes('https://sovereigntank.au/'), 'HTML canonical domain is sovereigntank.au'],
  [robots.includes('https://sovereigntank.au/sitemap.xml'), 'robots sitemap uses sovereigntank.au'],
  [sitemap.includes('https://sovereigntank.au/'), 'sitemap uses sovereigntank.au'],
  [security.includes('https://sovereigntank.au/.well-known/security.txt'), 'security.txt canonical uses sovereigntank.au'],
  [worker.includes("host === 'sovereigntank.au'") && worker.includes("host === 'www.sovereigntank.au'"), 'production host allow-list uses .au'],
  [wrangler.includes('ce77d11eb9c9640fe37ed06ecfe260aa'), 'existing Workers Builds account bound'],
  [!wrangler.includes('f7c8702f5256dcb45ef114533e872e2e'), 'stale cross-account A2 binding absent'],
